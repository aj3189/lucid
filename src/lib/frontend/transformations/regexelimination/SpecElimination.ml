...
let make_sr_macro vrid hid effect idx_exp_list =
  ...
  (*GIANT ID hack here*)
            | Some bs -> snd (List.find (fun (id, exp) -> String.equal (id_to_string id) (id_to_string hid)) bs) 
            | None -> (make_num 0)) in
...
let insert_if_necessary h_id sort body infos = 
  ...
  decl (DHandler (h_id, sort, ((fst body), statement (sequence_statements (List.append insertions [(snd body)])))))
...
let replacer = 
  object (self) 
    ...
    method! visit_DSpecRegex env id size sr = 
      match sr.s_regex with 
      | SRDetect(data, idx_exp_list, vr, effect) -> 
        print_endline ("num_events"^(string_of_int (List.length (get_events vr)))); env := {vr_id = id; events = (get_events vr); data = data; idx_exp_list = idx_exp_list; effect = effect} :: (!env); DSpecRegex (id, size, sr)
  end
